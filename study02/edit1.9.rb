...
results.each_with_index do |one_segment, i|
  one_segment.each_with_index do |duration, j|
    ...
    beat_index = i < results.length / 2 ? rand(last_beat_index / 2)
                                        : (last_beat_index / 2) + rand(last_beat_index / 2)
...
last_beats.inject(first_beat){|r, b| r + b} > "_results/edit1.9.#{Time.now.to_i}.wav"
puts "end"
